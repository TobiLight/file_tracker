...
export const Login = () => {
    ...
    const [, setUserPlanToLocalStorage] = useLocalStorage<PlanType | undefined>('ft_user_plan', undefined)
    const [token,] = useLocalStorage<string | undefined>('ft_token', undefined)
    const { getUser, setUserPlan } = useUserStore()
    const history = useHistory()

    useEffect(() => {
        if (token && (user?.email || getUser()?.email) && userHasPlan) {
            return history.replace("/user/dashboard/overview")
        }

        if (token && (user?.email || getUser()?.email) && !userHasPlan) {
            return history.replace("/select-plan")
        }
    ...
    const handleSubmit = (event: React.MouseEvent<HTMLElement> | React.FormEvent<HTMLFormElement>) => {
        ...
        LoginHandler.mutate({ ...input }, {
            onSuccess: (data: UserProfileType) => {
                if (data.has_plan) {
                    return getUserPlanData(data.user.email).then(userPlan => {
                        setUserPlan(userPlan)
                        setUserPlanToLocalStorage(userPlan)
            ...
            <div className="mt-40 mb-10">
                <div className="shadow-lg bg-white rounded w-10/12 sm:w-1/2 lg:w-2/5 mx-auto py-8 px-4">
                    ...
                    <form className="px-6 pt-8" onSubmit={handleSubmit}>
                        ...
                        <p className="text-center font-semibold mt-6">Don't have an account? <span className="font-normal"><NavLink className="text-blue-500 hover:text-blue-600" to="/register">Click here</NavLink></span></p>
                    </form>
                </div>
            </div>
